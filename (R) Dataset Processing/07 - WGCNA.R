#Note: Variable names are yet to be changed.
#DATA SOURCE: https://www.youtube.com/watch?v=dwWhm78j8YU
#GIRHUB: https://github.com/Lindseynicer/WGCNA_tutorial/blob/main/WGCNA_tutorial_Rscript
#=============================================================
...
setwd("~Users/keesh/OneDrive/Documents/Thesis_Life Depends on It/SVM-RFE-master")
...
data0=read.csv("FILTERED_data_mrna_seq_v2_rsem_.csv")
data1 <- data0[,-1]
...
sample_metadata = read.csv(file = "PATIENT_FILTERED.csv")

#skipped normalization since data is already normalized
datExpr = log(datExpr3+1)

head(datExpr[1:5,1:5])
match(sample_metadata$PATIENT, colnames(data0))
...
bac_traits = read.csv("PATIENT_FILTERED.csv")
...
pdf("5-module-traits-intersection.pdf", width = 40, height = 15)
par(mar = c(15, 12, 5, 5));
...
# Define variable
Verru = as.data.frame(bac_traits$AreaShape_Zernike_7_3);
names(Verru) = "AreaShape_Zernike_7_3"
...
par(cex = 1.0)
plotEigengeneNetworks(MET, "Eigengene adjacency heatmap", marHeatmap = c(10,15,1,2),
...
bubble_Data_lightgreen <- bubble_Data[which(bubble_Data$module == "MEturquoise"),]
